...
| Execution data             | `buildChatflow.ts` creates/stores chat messages and sends AgentFlow telemetry. Flowise telemetry is enabled unless `DISABLE_FLOWISE_TELEMETRY=true`. The pinned tool catalog includes custom tools, code-interpreter, filesystem, HTTP, and MCP capabilities. The inherited PR `main.yml` starts Flowise for Cypress without setting `DISABLE_FLOWISE_TELEMETRY`.                                                                                                                                                                                                                                                                                   | Atlas project, actor, assignment, review, governance, and event-outbox records must remain outside Flowise, as required by `ATLAS_UPSTREAM.md`. No Flowise instance or adapter transport runtime was started locally for this Phase-0 work; the disabled module is exercised only in-process by its contract test. Opening a PR would start the inherited localhost Flowise process (and the inherited Docker build) without Atlas credentials or production data, but with Flowise's default telemetry posture. Changing that inherited CI behavior is a separate GitHub Actions and telemetry security decision; do not create an Atlas Phase-0 PR until it is made. |
