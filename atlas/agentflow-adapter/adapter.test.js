...
const atlasAdapterReference =
    /agentflow-adapter\b|@atlas[\\/]|\\\\(?:[^\s\\/"'`]+[\\/])+atlas(?:[\\/]|(?=[\s"'`]|$))|\b[A-Za-z]:[\\/](?:[^\s"'`]*[\\/])?atlas(?:[\\/]|(?=[\s"'`]|$))|(?:^|[\s"'`}=])\\?\/(?:[^\s\/"'`]+\\?\/)*atlas(?:\\?\/|(?=[\s"'`]|$))|\b(?:require|import)\s*\(\s*["'`]atlas(?=["'`])|\bimport\s+["'`]atlas(?=["'`])|(?:^|[\s"'`=])(?:\.{1,2}[\\/])+atlas(?:[\\/]|["'`])|[?:]?=\s*atlas(?:[\\/]|(?=[\s"'`]|$))|\b(?:require|import)\s*\(\s*["'`][^"'`]*[\\/]atlas(?:[\\/]|["'`])|\bimport\s+["'`][^"'`]*[\\/]atlas(?:[\\/]|["'`])|\b(?:COPY|ADD)\s+(?:(?:\.{1,2})?[\\/])?atlas(?:[\\/\s"'`]|$)|\b(?:COPY|ADD)\s*\[\s*["'`]atlas["'`]|\bcp\s+(?:-[A-Za-z]+\s+)*(?:[^\s"'`]*[\\/])?atlas(?:[\\/\s"'`]|$)|\bworking-directory\s*:\s*(?:\.[\\/])?atlas(?:[\\/\s#]|$)/im
...
test('Flowise containment rejects unquoted relative Atlas paths after shell assignment operators', () => {
    for (const source of ['ATLAS_DIR=../atlas/bridge', 'ATLAS_DIR=./atlas/bridge']) {
        assert.throws(() => assertRuntimeSourceDoesNotReferenceAdapter('deploy.sh', source))
    }
})
...
test('Flowise containment rejects an absolute path ending at the Atlas boundary root', () => {
    for (const [name, source] of [
        ['deploy.ps1', 'Copy-Item -Recurse C:/repo/atlas C:/runtime\n'],
        ['deploy.sh', 'cp -R /repo/atlas /runtime\n']
    ]) {
        assert.throws(() => assertRuntimeSourceDoesNotReferenceAdapter(name, source))
    }
})
